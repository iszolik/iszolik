<?php
/**
 * Revolut Fizetési Plugin - Confirmation Form Template
 * 
...
<div id="sr-revolut-payment-form">
    <!-- Revolut fizetési form tartalma -->
    <div class="alert alert-info">
        <p><?php echo JText::_('PLG_SOLIDRESPAYMENT_REVOLUT_PROCESSING_PAYMENT'); ?></p>
    </div>
    
    <button id="revolut-pay-button" class="btn btn-primary">
        <?php echo JText::_('PLG_SOLIDRESPAYMENT_REVOLUT_PAY_NOW'); ?>
    </button>
</div>
...
(function() {
    ...
    /**
     * Revolut fizetés inicializálása Fetch API-val
     * 
     * Ez a függvény kezeli a Revolut fizetési folyamat elindítását,
     * promise-alapú hibakezeléssel.
     */
    function initiateRevolutPayment() {
        // Gomb letiltása dupla kattintás ellen
        var payButton = document.getElementById('revolut-pay-button');
        if (payButton) {
            payButton.disabled = true;
            payButton.textContent = 'Feldolgozás...';
        }
        
        // AJAX URL felépítése a Revolut fizetés inicializálásához
        ...
        .then(function(data) {
            ...
            if (data.success && data.payment_url) {
                // Átirányítás a Revolut fizetési oldalra
        ...
        .catch(function(error) {
            ...
            console.error('Revolut fizetési hiba:', error);
            alert('Hiba történt a fizetés során. Kérjük, próbálja újra később.');
            
            // Gomb újra engedélyezése
            if (payButton) {
                payButton.disabled = false;
                payButton.textContent = '<?php echo JText::_('PLG_SOLIDRESPAYMENT_REVOLUT_PAY_NOW'); ?>';
            }
        });
    }
    
    /**
     * Revolut callback kezelése sikeres fizetés után
     * 
     * Ez a függvény fut le, amikor a felhasználó visszatér
     * a Revolut fizetési oldalról.
     */
    function handleRevolutCallback() {
        // URL paraméterek ellenőrzése
        var currentParams = new URLSearchParams(window.location.search);
        
        // Ellenőrizzük, hogy Revolut callback-ről van-e szó
        if (currentParams.has('revolut_status') || currentParams.has('payment_status')) {
            var status = currentParams.get('revolut_status') || currentParams.get('payment_status');
            ...
            .catch(function(error) {
                console.error('Revolut callback hiba:', error);
